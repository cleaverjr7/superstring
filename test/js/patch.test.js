...
describe('Patch', function () {
    ...
    assert.deepEqual(JSON.parse(JSON.stringify(patch.getChanges())), [
      {
        ...
        newEnd: { row: 0, column: 7 }
      },
      {
        oldStart: { row: 0, column: 9 },
        oldEnd: { row: 0, column: 12 },
        newStart: { row: 0, column: 10 },
        newEnd: { row: 0, column: 14 }
      }
    ])
      ...
      assert.deepEqual(JSON.parse(JSON.stringify(composedPatch.getChanges())), [
        {
          ...
          newEnd: { row: 3, column: 0 },
          oldText: '',
          newText: '\n\n\n'
        },
        {
          oldStart: { row: 0, column: 3 },
          oldEnd: { row: 0, column: 7 },
          newStart: { row: 3, column: 3 },
          newEnd: { row: 3, column: 8 },
          oldText: 'ciao',
          newText: 'hello'
        },
        {
          oldStart: { row: 0, column: 14 },
          oldEnd: { row: 0, column: 24 },
          newStart: { row: 3, column: 15 },
          newEnd: { row: 3, column: 15 },
          oldText: '0123456789',
          newText: ''
        },
        {
          oldStart: { row: 1, column: 1 },
          oldEnd: { row: 1, column: 1 },
          newStart: { row: 4, column: 1 },
          newEnd: { row: 4, column: 4 },
          oldText: '',
          newText: 'hho'
        }
      ])
      ...
      assert.deepEqual(JSON.parse(JSON.stringify(composedPatch.getChanges())), [
        {
          oldStart: { row: 1, column: 1 },
          oldEnd: { row: 1, column: 1 },
          newStart: { row: 1, column: 1 },
          newEnd: { row: 1, column: 4 },
          oldText: '',
          newText: 'hey'
        }
      ])
  ...
  it('can invert patches', function () {
    ...
    assert.deepEqual(JSON.parse(JSON.stringify(invertedPatch.getChanges())), [
      ...
      {
        oldStart: { row: 0, column: 10 },
        oldEnd: { row: 0, column: 15 },
        newStart: { row: 0, column: 9 },
        newEnd: { row: 0, column: 14 },
        oldText: 'world',
        newText: 'quick'
      }
  ...
  it('correctly records random splices', function () {
    ...
    for (let i = 0; i < 100; i++) {
      ...
      for (let j = 0; j < 20; j++) {
        ...
        for (let k = 0; k < 5; k++) {
          const oldRange = originalDocument.buildRandomRange()
          assert.deepEqual(
          ...
          const newRange = mutatedDocument.buildRandomRange()
          assert.deepEqual(
          ...
          const oldPoint = originalDocument.buildRandomPoint()
          assert.deepEqual(
          ...
          const newPoint = mutatedDocument.buildRandomPoint()
          assert.deepEqual(
